...
import { z } from 'zod';
import { MODEL, EXTRACTION_INSTRUCTIONS, EVIDENCE_INSTRUCTIONS, VERIFICATION_INSTRUCTIONS } from '@/app/config/constants';
    ...
    try {
        ...
        const evidenceBundle = await client.responses.parse({
            ...
            input: [
                {
                    role: "user",
                    content: JSON.stringify({ claimsList, policy: { time_window_days: 365 } })
                }
            ],
            tools: [{
                type: "function",
